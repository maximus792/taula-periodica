...
$(document).ready(function () {
	...
	function newElement(n) {
		...
		sol.forEach((element) => {
            ...
            let r = Math.floor(Math.random() * lettersf.length);
            if(lettersf[r] != letters[n])
                element.innerHTML = namesf[r];
            else{
                let r = Math.floor(Math.random() * lettersf.length);
                element.innerHTML = namesf[r];
            }
		...
		sol[Math.floor(Math.random() * 5)].innerHTML = names[n];

		sol.forEach((element) => {
			$(element).unbind().click(function (event) {

				//console.log(element.innerHTML);
				if (element.innerHTML === names[n]) {
                    $(element).removeClass("incorrect");
